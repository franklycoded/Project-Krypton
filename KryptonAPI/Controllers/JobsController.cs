...
namespace KryptonAPI.Controllers
{
    ...
    public class JobsController : Controller
    {
        ...
        static JobsController() {
            ...
            {
                _jobs.Enqueue(new JobDto(){
                    Id = i,
                    Status = "Ready",
                    Task = GetTask(i, _taskNum)
                });
            }
        }
        ...
        [HttpPut("{id}")]
        [Authorize(ActiveAuthenticationSchemes = "Bearer")]
        public IActionResult PutResult(long id, [FromBody] JobDto jobResult){
            lock(_lock){
        ...
        private static string GetTask(int taskIndex, int divider){
            ...
            var commands = "var w = " + w + "; var h = " + h + "; var rowStart = " + startRow + "; var rowFinish = " + endRow + "; var data = getImageData(scene, w, h, rowStart, rowFinish); postMessage({results: data});";
